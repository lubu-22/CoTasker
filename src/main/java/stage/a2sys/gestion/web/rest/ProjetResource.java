...
/**
 * REST controller for managing {@link stage.a2sys.gestion.domain.Projet}.
 */
@RestController
@RequestMapping("/api")
public class ProjetResource {
     ...
     */
    @GetMapping("/projets")
    public ResponseEntity<List<ProjetDTO>> getAllProjets(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Projets");
        Page<ProjetDTO> page = projetService.findAll(pageable);
    ...
    @GetMapping("/avoirDossier/{id}")
    public List<DossierDTO> AvoirDossierDeProjet(@PathVariable Long id) {
        log.debug("REST request to get all dossier of a Projet");
        return projetService.AvoirDossierServiceImpl(id);
    }

    @GetMapping("/avoirTache/{id}")
    public List<TacheDTO> AvoirTacheDeProjet(@PathVariable Long id) {
        log.debug("REST request to get all tache of a Projet");
        return projetService.AvoirTacheServiceImpl(id);
    }
